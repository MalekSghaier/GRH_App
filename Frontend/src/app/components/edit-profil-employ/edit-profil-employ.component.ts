...
@Component({
  selector: 'app-edit-profil-employ',
  imports: [CommonModule, RouterModule,ReactiveFormsModule,SharedNavbarComponent,SharedSidebarComponent],
  templateUrl: './edit-profil-employ.component.html',
  styleUrl: './edit-profil-employ.component.css',
  encapsulation: ViewEncapsulation.None // Désactive l'encapsulation

})
export class EditProfilEmployComponent implements AfterViewInit, OnInit {
  currentRoute: string = '';
  ...
  navigateToEditProfile(): void {
    this.router.navigate(['/edit-profile-employ']);
  }
    ...
    if (this.editProfileForm.valid) {
      this.userService.updateProfile(this.editProfileForm.value).subscribe(
        (data) => {
          ...
          setTimeout(() => {
            this.router.navigate(['/profil-employ']); 
          }, 700); 
        },
        (error) => {
  ...
  cancel(): void {
    this.router.navigate(['/profile-employ']);
  }
